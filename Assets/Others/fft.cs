...
public static class FFTHelper
{
    ...
    public static void tfftForHDN(Vector2[,] htilde, int count, out Vector2[,] hs,
        ...
        out Vector2[,] bxs, out Vector2[,] bys, out Vector2[,] bzs,
        out Vector2[,] txs, out Vector2[,] tys, out Vector2[,] tzs)
    {
        ...
        bxs = new Vector2[count, count];
        bys = new Vector2[count, count];
        bzs = new Vector2[count, count];
        txs = new Vector2[count, count];
        tys = new Vector2[count, count];
        tzs = new Vector2[count, count];
        Vector2 k;
        Vector2[] cntemp = new Vector2[count];
        float kx, kz;
        int groupnum = count;
        ...
        for (int x = 0; x < count; x++)
        {
            ...
            for (int y = 0; y < count; y++)
            {
                ...
                //这里是利用FFT做的IFFT
                k = new Vector2(kx, kz);
                hs[x, y].x = htilde[x, y].x;
                hs[x, y].y = -htilde[x, y].y;
                ...
                bys[x, y].x = -htilde[x, y].y * kx;
                bys[x, y].y = -htilde[x, y].x * kx;
                tys[x, y].x = -htilde[x, y].y * kz;
                tys[x, y].y = -htilde[x, y].x * kz;
                ...
                else
                {
                    ...
                    bxs[x, y].x = htilde[x, y].x * kx * kx / k.magnitude;
                    bxs[x, y].y = -htilde[x, y].y * kx * kx / k.magnitude;
                    bzs[x, y].x = htilde[x, y].x * kx * kz / k.magnitude;
                    bzs[x, y].y = -htilde[x, y].y * kx * kz / k.magnitude;
                    txs[x, y].x = htilde[x, y].x * kx * kz / k.magnitude;
                    txs[x, y].y = -htilde[x, y].y * kx * kz / k.magnitude;
                    tzs[x, y].x = htilde[x, y].x * kz * kz / k.magnitude;
                    tzs[x, y].y = -htilde[x, y].y * kz * kz / k.magnitude;
                }
            }
        }
        FFT(hs, count);
